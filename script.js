...
noBtn.onmouseenter = noBtn.onclick = () => {
    ...
    noBtn.style.left = Math.random() * (innerWidth - 320) + 60 + 'px';
    noBtn.style.top = Math.random() * (innerHeight - 220) + 60 + 'px';
    noBtn.style.animation = 'vibrate 0.9s';
    yesBtn.classList.add('glow');
    setTimeout(() => yesBtn.classList.remove('glow'), 1400);
...
yesBtn.onclick = () => {
    ...
    typewriter.style.opacity.Op = subtext.style.opacity = trollNote.style.opacity = 0;
...
function loadScratch() {
    ...
    img.onload = () => {
        ...
        document.querySelector('.final-note').style.opacity = 1;
